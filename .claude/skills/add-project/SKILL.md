---
name: add-project
description: 홈페이지에 새 Roblox 프로젝트를 추가한다. data/projects.json 메타, 상세 셸 HTML, js/project-details/<slug>.js 콘텐츠, 표시 순서, 에셋을 순서대로 만들고 검증한다. "프로젝트 추가", "새 게임 페이지", "상세 페이지 만들어줘" 요청에 사용.
---
...
## 0. 사전 확인
- slug 는 kebab-case, 기존 `data/projects.json`의 `id`와 겹치지 않아야 한다.
...
## 1. 에셋
- `assets/<slug-without-dash>/`에 미리보기(`*-preview.jpg`), 메인(`*-main.jpg`), 갤러리 이미지를 넣는다. 가로 1200px 이하, 2MB 이하.

## 2. 데이터 (`data/projects.json`)
...
## 4. 콘텐츠 설정 (`js/project-details/<slug>.js`)
...
## 5. 표시 순서
- `js/project-renderer.js`의 `projectDisplayOrder`에 slug 를 원하는 위치에 넣는다.
