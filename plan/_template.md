# {프로젝트명} 상세 페이지 작업 계획 (TEMPLATE)

> 본 파일은 페이지별 계획 문서의 표준 골격입니다. 각 상세 페이지 계획서는 이 구조를 그대로 복제해 사용합니다.
> 사용자에게 노출되는 모든 텍스트 필드는 KO / EN / JA 3개 언어로 채워야 합니다.

- **slug**: `{slug}`
- **HTML**: `{slug}.html`
- **콘텐츠 설정 파일**: `js/project-details/{slug}.js`
- **카드 메타 위치**: `data/projects.json` (id: `{slug}`)
...
- [ ] `js/project-details/{slug}.js` 콘텐츠 입력 (KO / EN / JA)
